...
def down(rtl, isSpecial):
	...
	if r.status_code == 404:
		if isSpecial:
			logging.info("检查到 R{} 的图片服务器中不存在".format(rtl))
		return
	if r.headers['Last-Modified'] != rmod:
		logging.info("监测到 R{} 有更新，正在保存图片".format(rtl))
		savename = rpath + "R" + rtl + "_" + r.headers['Last-Modified'].replace(" ", "").replace(",", "").replace(":", "") + ".png"
		os.system("wget -O {} {}".format(savename, base))
		try: 
		...
		tellRaw = "{} Apple {} (R{}) 图片更新\n修改标签 {}，{}".format(rflag, rname, rtl, r.headers['Last-Modified'], rnso)
		...
		try: 
			pname = "Apple {} (R{})".format(storejson['name'][rtl], rtl)
		except KeyError: 
			pname = "Apple Store (R{})".format(rtl)  
		logging.info("检查到 {} 的图片没有更新".format(pname))
...
if sys.argv[1] == "normal":
	...
	for j in range(1, totalStore):
		down("{:0>3d}".format(j), False)
		if j == a:
			logging.info("已完成 {:.0f}/9".format(a / 100))
			a += 100
	logging.info("程序结束")
